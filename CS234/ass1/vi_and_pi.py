...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    #https://www.cs.upc.edu/~mmartin/Ag4-4x.pdf
    #http://web.stanford.edu/class/cs234/slides/lecture2.pdf
    ...
    while True:
        ...
        for state in range(nS):
            for probability, nextstate, reward, terminal in P[state][policy[state]]:
                v[state] += probability * value_function[nextstate]
            v[state] = reward + gamma * v[state]
        if np.max(np.abs(v - value_function)) < tol:
    ...
    for state in range(nS):
        for action in range(nA):
            for probability, nextstate, reward, terminal in P[state][action]:
                q[state][action] += probability * value_from_policy[nextstate]
            q[state][action] = reward + gamma * q[state][action]
    ...
    while np.max(np.abs(value_function - prev_value_function)) > tol:
        ...
        for state in range(nS):
            ...
            for action in range(nA):
                ...
                for probability, nextstate, reward, terminal in P[state][action]:
                    value_function[state] += probability * prev_value_function[nextstate]
                action_vector.append( reward +  gamma * value_function[state])
            value_function[state] = np.max( action_vector )
